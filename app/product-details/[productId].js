...
import { View, Text, ScrollView, Modal, TouchableOpacity, Pressable, StyleSheet, Image } from 'react-native';
...
export default function ProductDetailScreen() {
  ...
  //   console.log(`Navigating to 192.168.1.233:8081/edit/${productId}`)
  //   router.navigate(`/edit/${productId}`)
  // }

  const handleClearImage = async () => {
    const token = await AsyncStorage.getItem('token')
    axios.delete(`http://192.168.1.233:5001/product/${productId}/clear-image`, {
        headers: {
            'Authorization': `Bearer ${token}`
        }
    })
    setModalVisible(false)
    ToastComponent('success', 'Success', 'Successfully cleared image')
  }

  const handleDelete = async () => {
    const token = await AsyncStorage.getItem('token')
    axios.delete(`http://192.168.1.233:5001/api/products/${productId}`, {
        headers: {
            'Authorization': `Bearer ${token}`
        }
    })
    setDeleteModalVisible(false)
    ToastComponent('success', 'Success', 'Deletion successful')
  }

  const openClearImageModal = async () => {
  ...
  const closeClearImageModal = async () => {
    setModalVisible(false)
  }
    ...
    <SafeAreaProvider>
      <SafeAreaView style={styles.page}>
        <TouchableOpacity onPress={() => {
            ...
            <Ionicons name="arrow-back" size={24} color="#007aff" style={{ marginTop: 20, marginLeft: 20 }} />
        </TouchableOpacity>
